...
if aligner == "bwa_sentieon":
    rule fix_af:
        input:
            vcf="sentieon/tnscope/{sample}_TNscope_tn_ML.filter.germline.vcf",
        output:
            vcf=temp("sentieon/tnscope/{sample}_TNscope_tn_ML.filter.germline.fix_af.vcf"),
        log:
            "sentieon/tnscope/{sample}_TNscope_tn_ML.fix_af.log",
        benchmark:
            repeat(
                "sentieon/tnscope/{sample}_TNscope_tn_ML.filter.germline.fix_af.benchmark.tsv",
                config.get("fix_af", {}).get("benchmark_repeats", 1),
            )
        threads: config.get("fix_af", {}).get("threads", config["default_resources"]["threads"])
        resources:
